...
path = './data/blca/'
data = pd.read_csv(path + "single_cancer_blca.csv", sep=",").values[:, :3]
...
def train(mask, Y):
    ...
    optimizer.zero_grad()
    pred, pred1 = model()
    loss = 0.5 * F.binary_cross_entropy_with_logits(pred[mask], Y[mask]) + \
           0.5 * F.binary_cross_entropy_with_logits(pred1[mask], Y[mask])
    loss.backward()
    optimizer.step()
...
class Net(torch.nn.Module):
    def __init__(self, num_feature=3, num_label=1, hidden_channels_2=64, num_layers=6, alpha=0.1, theta=0.4,
                 dropout=0.1):
