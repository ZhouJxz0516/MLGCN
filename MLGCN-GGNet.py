...
class Net(torch.nn.Module):
    def __init__(self, num_feature=data.x.shape[1], num_label=1, hidden_channels_2=8, num_layers=8,
                 alpha=0.85, theta=1.5, dropout=0.4):
...
pred_all = np.zeros((data.x.shape[0], 1))
for i in range(10):
    for cv_run in range(5):
        ...
        for epoch in range(1, EPOCH):
            ...
            optimizer.zero_grad()
            pred, pred1 = model()
            loss = 0.8*F.binary_cross_entropy_with_logits(pred[tr_mask], data.y[tr_mask].view(-1, 1))\
            + 0.2*F.binary_cross_entropy_with_logits(pred1[tr_mask], data.y[tr_mask].view(-1, 1))
            loss.backward()
            optimizer.step()
